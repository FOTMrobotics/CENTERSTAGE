...
public class RedAuto extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        int waitTime = 500;
        OpenCvWebcam webcam;
        TeamElementDetectionRed pipeline;

        Pose2d start = new Pose2d(-39.5, -62, Math.toRadians(270));
        ...
        pipeline = new TeamElementDetectionRed();
        ...
        TrajectorySequence toBoard = drive.trajectorySequenceBuilder(new Pose2d(38, -37, Math.toRadians(180)))
                .lineToLinearHeading(new Pose2d(53.9, ypos, Math.toRadians(180)))
                .build();
        ...
        TrajectorySequence backup = drive.trajectorySequenceBuilder(toBoard.end())
                .lineToLinearHeading(new Pose2d(38, ypos, Math.toRadians(180)))
                .build();
        ...
        TrajectorySequence toPark = drive.trajectorySequenceBuilder(backup.end())
...
class TeamElementDetectionRed extends OpenCvPipeline {
    Mat YCbCr = new Mat();
    Mat leftCrop;
    Mat rightCrop;
    double leftAvgFinal;
    ...
    public Mat processFrame(Mat input) {
        ...
        Rect leftRect = new Rect(1, 130, 200, 160);
        Rect rightRect = new Rect(300, 130, 200, 160);
        ...
        leftAvgFinal = leftAvg.val[0] - 5;
        rightAvgFinal = rightAvg.val[0];

        if (leftAvgFinal > rightAvgFinal) {
            pos = 1;
        } else if (leftAvgFinal - rightAvgFinal < 8 && leftAvgFinal - rightAvgFinal > -8) {
            pos = 3;
        } else {
            pos = 2;
        }
